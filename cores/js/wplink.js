/**
 * @output matindia-includes/js/wplink.js
 */
...
( function( $, wpLinkL10n, wp ) {
	...
	window.wpLink = {
		...
		init: function() {
			inputs.wrap = $('#matindia-link-wrap');
			inputs.dialog = $( '#matindia-link' );
			inputs.backdrop = $( '#matindia-link-backdrop' );
			inputs.submit = $( '#matindia-link-submit' );
			inputs.close = $( '#matindia-link-close' );

			// Input.
			inputs.text = $( '#matindia-link-text' );
			inputs.url = $( '#matindia-link-url' );
			inputs.nonce = $( '#_ajax_linking_nonce' );
			inputs.openInNewTab = $( '#matindia-link-target' );
			inputs.search = $( '#matindia-link-search' );
			...
			inputs.close.add( inputs.backdrop ).add( '#matindia-link-cancel button' ).on( 'click', function( event ) {
				event.preventDefault();
				wpLink.close();
			});
		...
		mceUpdate: function() {
			...
			editor.undoManager.transact( function() {
				if ( ! $link.length ) {
					editor.execCommand( 'mceInsertLink', false, { href: '_wp_link_placeholder', 'data-matindia-temp-link': 1 } );
					$link = editor.$( 'a[data-matindia-temp-link="1"]' ).removeAttr( 'data-matindia-temp-link' );
		...
		keydown: function( event ) {
			...
			// Tab key.
				...
				// matindia-link-submit must always be the last focusable element in the dialog.
				// Following focusable elements will be skipped on keyboard navigation.
				if ( id === 'matindia-link-submit' && ! event.shiftKey ) {
					inputs.close.trigger( 'focus' );
					event.preventDefault();
				} else if ( id === 'matindia-link-close' && event.shiftKey ) {
	...
	River = function( element, search ) {
		...
		$( '#matindia-link .query-results, #matindia-link #link-selector' ).on( 'scroll', function() {
			self.maybeLoad();
		});
