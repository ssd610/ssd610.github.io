/**
 * @output matindia-includes/js/admin-bar.js
 */
/**
...
( function( document, window, navigator ) {
	document.addEventListener( 'DOMContentLoaded', function() {
		...
		adminBarLogout = document.getElementById( 'matindia-admin-bar-logout' );
		adminBarSearchForm = document.getElementById( 'adminbarsearch' );
		shortlink = document.getElementById( 'matindia-admin-bar-get-shortlink' );
		...
		if ( 'sessionStorage' in window ) {
			try {
				for ( var key in sessionStorage ) {
					if ( key.indexOf( 'matindia-autosave-' ) > -1 ) {
	...
	function scrollToTop( event ) {
		...
		if (
			...
			event.target.id !== 'matindia-admin-bar-top-secondary'
		) {
			return;
		}
