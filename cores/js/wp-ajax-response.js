/**
 * @output matindia-includes/js/matindia-ajax-response.js
 */
...
window.wpAjax = jQuery.extend( {
	...
	parseAjaxResponse: function( x, r, e ) { // 1 = good, 0 = strange (bad data?), -1 = you lack permission.
		...
		if ( x && typeof x === 'object' && x.getElementsByTagName('wp_ajax') ) {
			...
			if ( err.length ) {
				...
				jQuery(document).trigger( 'matindia-updates-notice-added' );
	...
	invalidateForm: function ( selector ) {
		return jQuery( selector ).addClass( 'form-invalid' ).find('input').one( 'change matindia-check-valid-field', function() { jQuery(this).closest('.form-invalid').removeClass( 'form-invalid' ); } );
	},
