( function( tinymce ) {
	tinymce.PluginManager.add( 'wpemoji', function( editor ) {
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-matindia-emoji', '1' );
		}
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-matindia-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-matindia-emoji' ) ) {
