...
tinymce.PluginManager.add( 'wpeditimage', function( editor ) {
		...
		editor.addButton( 'wp_img_' + name, {
			...
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.matindia-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:wp_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:wp_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			return '<div class="mceTemp"><dl id="' + id + '" class="matindia-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="matindia-caption-dt">'+ img +'</dt><dd class="matindia-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+matindia-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /matindia-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^matindia-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'matindia-image-', '' ), 10 );
		...
		// Extract caption.
		captionBlock = dom.getParents( imageNode, '.matindia-caption' );
		...
		if ( captionBlock.length ) {
			...
			tinymce.each( classes, function( name ) {
				if ( /^align/.test( name ) ) {
					metadata.align = name.replace( 'align', '' );
				} else if ( name && name !== 'matindia-caption' ) {
			...
			caption = dom.select( 'dd.matindia-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( $imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'matindia-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			className = 'matindia-caption ' + align;
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.matindia-caption', captionNode );
				...
				dd = dom.select( '.matindia-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="matindia-caption-dt"></dt><dd class="matindia-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.matindia-caption-dt' ).append( node );
	...
	function editImage( img ) {
		...
		editor.$( img ).attr( 'data-matindia-editing', 1 );
		...
		frame.on( 'close', function() {
			 ...
			 */
			imageNode = editor.$( 'img[data-matindia-editing]' )
			imageNode.removeAttr( 'data-matindia-editing' );
		});
	...
	editor.on( 'init', function() {
		...
		// Prevent IE11 from making dl.matindia-caption resizable.
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+.
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.matindia-caption' ) ) {
					// Hide the thick border with resize handles around dl.matindia-caption.
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'matindia-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.matindia-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.matindia-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		if ( cmd === 'mceInsertContent' || cmd === 'Indent' || cmd === 'Outdent' ) {
			...
			DL = editor.dom.getParent( node, '.matindia-caption' );
	...
	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__matindia-temp-img-id"]' ).removeAttr( 'id' );
		}
	});
