...
tinymce.PluginManager.add( 'wpautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'matindia-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'matindia-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'matindia-autoresize' );
	...
	function off() {
		...
		if ( ! settings.wp_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'matindia-autoresize' );
