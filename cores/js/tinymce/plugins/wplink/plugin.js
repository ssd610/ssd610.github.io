( function( tinymce ) {
	tinymce.ui.Factory.add( 'WPLinkPreview', tinymce.ui.Control.extend( {
		...
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="matindia-link-preview">' +
	...
	tinymce.ui.Factory.add( 'WPLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="matindia-link-input">' +
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-matindia-autocomplete-' + ui.item.ID );
							/*
						...
						return $( '<li role="option" id="mce-matindia-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="matindia-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
			...
			if ( linkNode ) {
				...
				if ( href === '_wp_link_placeholder' || edit ) {
					...
					if ( $linkNode.attr( 'data-wplink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.matindia-link-preview a' ).addClass( 'wplink-url-error' );
					} else {
						toolbar.$el.find( '.matindia-link-preview a' ).removeClass( 'wplink-url-error' );
						hasLinkError = false;
					}
				}
