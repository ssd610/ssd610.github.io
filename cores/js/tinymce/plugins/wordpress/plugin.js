...
tinymce.PluginManager.add( 'wordpress', function( editor ) {
	...
	function toggleToolbars( state ) {
		...
		editor.fire( 'matindia-toolbar-toggle' );
	}
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-matindia-more="more" data-matindia-more-text="' + moretext + '" ' +
						'class="matindia-more-tag mce-matindia-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-matindia-more="nextpage" class="matindia-more-tag mce-matindia-nextpage" ' +
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-matindia-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-matindia-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-matindia-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-matindia-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-matindia-preserve' ) !== -1 ) {
					if ( match = image.match( / data-matindia-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-matindia-more' ) ) ) {
			event.name = attr;
		}
	});
	...
	editor.addCommand( 'WP_More', function( tag ) {
		var parent, html, title,
			classname = 'matindia-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode(),
			rootNode = editor.getBody();

		tag = tag || 'more';
		classname += ' mce-matindia-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
			'data-matindia-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'WP_Help', function() {
		...
		html = '<div class="matindia-editor-help">';

		// Main section, default and additional shortcuts.
		html = html +
			'<h2>' + __( 'Default shortcuts,' ) + ' ' + meta + '</h2>' +
			'<table class="matindia-help-th-center fixed">' +
				header +
				table1.join('') +
			'</table>' +
		...
		if ( editor.plugins.wptextpattern && ( ! tinymce.Env.ie || tinymce.Env.ie > 8 ) ) {
			...
			html = html +
				...
				'<table class="matindia-help-single">' +
					tr({ '>': 'Blockquote' }) +
					tr({ '##': 'Heading 2' }) +
					tr({ '###': 'Heading 3' }) +
					tr({ '####': 'Heading 4' }) +
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-matindia-help' );
	...
	editor.on( 'init', function() {
		...
		bodyClass.push('matindia-editor');
