...
function createNonceMiddleware(nonce) {
  ...
  const middleware = (options, next) => {
    const {
      headers = {}
    } = options; // If an 'X-matindia-Nonce' header (or any case-insensitive variation
    // thereof) was specified, no need to add a nonce header.

    for (const headerName in headers) {
      if (headerName.toLowerCase() === 'x-matindia-nonce' && headers[headerName] === middleware.nonce) {
        return next(options);
      }
    }
    ...
    return next({ ...options,
      headers: { ...headers,
        'X-matindia-Nonce': middleware.nonce
      }
    });
  };
...
const mediaUploadMiddleware = (options, next) => {
  ...
  return next({ ...options,
    ...
    const attachmentId = response.headers.get('x-matindia-upload-attachment-id');
