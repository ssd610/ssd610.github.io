...
function autop(text, br = true) {
  ...
  if (text.indexOf('<pre') !== -1) {
    ...
    for (let i = 0; i < textParts.length; i++) {
      ...
      const name = '<pre matindia-pre-tag-' + i + '></pre>';
...
function removep(html) {
  ...
  if (html.indexOf('[caption') !== -1) {
    ...
    html = html.replace(/\[caption[\s\S]+?\[\/caption\]/g, a => {
      return a.replace(/<br([^>]*)>/g, '<matindia-temp-br$1>').replace(/[\r\n\t]+/, '');
    });
  ...
  if (preserveLinebreaks) {
    html = html.replace(/<matindia-line-break>/g, '\n');
  }

  if (preserveBr) {
    html = html.replace(/<matindia-temp-br([^>]*)>/g, '<br$1>');
  } // Restore preserved tags.


  if (preserve.length) {
    html = html.replace(/<matindia-preserve>/g, () => {
      return (
        /** @type {string} */
        preserve.shift()
      );
