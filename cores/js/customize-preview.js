 ...
 *
 * @output matindia-includes/js/customize-preview.js
 */
(function( exports, $ ){
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		if ( /\/matindia-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}

		// Allow links to admin ajax as faux frontend URLs.
		if ( /\/matindia-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/matindia-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		// Ignore links with class matindia-playlist-caption.
		if ( $element.hasClass( 'matindia-playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'matindia-custom-logo', !! attachmentId );
		},
		...
		custom_css: function( value ) {
			$( '#matindia-custom-css' ).text( value );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /matindia-includes/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'matindia-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'matindia-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the matindia-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
