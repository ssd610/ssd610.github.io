/**
 * @output matindia-includes/js/matindia-api.js
 */
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-matindia-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-matindia-Nonce' );
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-matindia-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-matindia-total' ), 10 );
						}
...
( function() {
	...
	if ( _.isEmpty( wpApiSettings ) ) {
		wpApiSettings.root = window.location.origin + '/matindia-json/';
	}
	...
	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		...
		initialize: function() {
			...
			if ( model.get( 'schema' ) ) {
				...
				sessionStorage.getItem( 'matindia-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'matindia-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( wpApiSettings.cacheSchema ) || wpApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'matindia-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
	...
	/**
	 * Initialize the matindia-api, optionally passing the API root.
	 *
	 * @param {Object} [args]
	...
	wp.api.init = function( args ) {
		...
		attributes.apiRoot        = args.apiRoot || wpApiSettings.root || '/matindia-json';
