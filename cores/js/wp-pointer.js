/**
 * @output matindia-includes/js/matindia-pointer.js
 */

/**
 * Initializes the matindia-pointer widget using jQuery UI Widget Factory.
 */
(function($){
	var identifier = 0,
		zindex = 9999;
	...
	$.widget('wp.pointer',/** @lends $.widget.wp.pointer.prototype */{
		...
		_create: function() {
			...
			this.content = $('<div class="matindia-pointer-content"></div>');
			this.arrow   = $('<div class="matindia-pointer-arrow"><div class="matindia-pointer-arrow-inner"></div></div>');
			...
			this.pointer = $('<div />')
				.append( this.content )
				.append( this.arrow )
				.attr('id', 'matindia-pointer-' + identifier++)
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="matindia-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /matindia-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'matindia-pointer-' + edge );
		},
