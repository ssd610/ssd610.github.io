 ...
 *
 * @output matindia-includes/js/matindia-auth-check.js
 */
...
( function( $ ) {
	 ...
	 */
	function show() {
		var parent = $( '#matindia-auth-check' ),
			form = $( '#matindia-auth-check-form' ),
			noframe = wrap.find( '.matindia-auth-fallback-expired' ),
			frame, loaded = false;

		if ( form.length ) {
			// Add unload confirmation to counter (frame-busting) JS redirects.
			$( window ).on( 'beforeunload.matindia-auth-check', function( event ) {
			...
			frame = $( '<iframe id="matindia-auth-check-frame" frameborder="0">' ).attr( 'title', noframe.text() );
	...
	function hide() {
		...
		$( window ).off( 'beforeunload.matindia-auth-check' );
		...
		wrap.fadeOut( 200, function() {
			...
			$( '#matindia-auth-check-frame' ).remove();
	...
	$( function() {
		 ...
		 * @since 3.6.0
		 */
		wrap = $( '#matindia-auth-check-wrap' );
		wrap.find( '.matindia-auth-check-close' ).on( 'click', function() {
			hide();
			setShowTimeout();
		});
	}).on( 'heartbeat-tick.matindia-auth-check', function( e, data ) {
		if ( 'matindia-auth-check' in data ) {
			if ( ! data['matindia-auth-check'] && wrap.hasClass( 'hidden' ) && ! tempHidden ) {
				show();
			} else if ( data['matindia-auth-check'] && ! wrap.hasClass( 'hidden' ) ) {
				hide();
			}
		}
	});
