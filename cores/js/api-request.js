 ...
 *
 * Currently only applies to requests that do not use the `matindia-api.js` Backbone
 ...
 * @output matindia-includes/js/api-request.js
 */
...
( function( $ ) {
	...
	apiRequest.buildAjaxOptions = function( options ) {
		...
		for ( headerName in headers ) {
			...
			// If an 'X-matindia-Nonce' or 'Accept' header (or any case-insensitive variation
			// thereof) was specified, no need to add the header again.
			switch ( headerName.toLowerCase() ) {
				case 'x-matindia-nonce':
		...
		if ( addNonceHeader ) {
			...
			headers = $.extend( {
				'X-matindia-Nonce': wpApiSettings.nonce
			}, headers );
		}
