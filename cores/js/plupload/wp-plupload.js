...
( function( exports, $ ) {
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__matindia-uploader-id-' + Uploader.uuid++ );
			}
		...
		tryAgain = function( message, data, file ) {
			...
			id = data.responseHeaders.match( /x-matindia-upload-attachment-id:\s*(\d+)/i );
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.matindia-uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.on( 'dragover.matindia-uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.on('dragleave.matindia-uploader, drop.matindia-uploader', function() {
				/*
	...
	$.extend( Uploader.prototype, /** @lends wp.Uploader.prototype */{
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				 ...
				 */
				if ( ! attached ) {
					id = 'matindia-uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="matindia-uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'matindia-uploader-browser-' + this.uploader.id ).appendTo('body');
					}
