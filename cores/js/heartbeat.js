 ...
 *
 * @since 3.6.0
 * @output matindia-includes/js/heartbeat.js
 */
	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.matindia-heartbeat', function() {
			...
			$(window).on( 'unload.matindia-heartbeat', function() {
		...
		function connect() {
			...
			settings.xhr = $.ajax({
				...
				// Update the Rest API nonce if set and matindia-api loaded.
		...
		function userIsActive() {
			...
			$document.off( '.matindia-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.matindia-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.matindia-heartbeat-active keyup.matindia-heartbeat-active touchend.matindia-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.matindia-heartbeat-active keyup.matindia-heartbeat-active touchend.matindia-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
