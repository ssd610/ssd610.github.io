/**
 * @output matindia-includes/js/matindia-lists.js
 */
...
wpList = {
	 ...
	 *
	 * Example 1: data-matindia-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-matindia-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		...
		try {
			wpListsData = $( element ).data( 'matindia-lists' ) || '';
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-matindia-lists^="add:' + list.id + ':"]', function() {
			return list.wpList.add( this );
		} );

		$element.on( 'click', '[data-matindia-lists^="add:' + list.id + ':"], input[data-matindia-lists^="add:' + list.id + ':"]', function() {
			return list.wpList.add( this );
		} );

		$element.on( 'click', '[data-matindia-lists^="delete:' + list.id + ':"]', function() {
			return list.wpList.del( this );
		} );

		$element.on( 'click', '[data-matindia-lists^="dim:' + list.id + ':"]', function() {
