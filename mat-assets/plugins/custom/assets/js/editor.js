...
module.exports = ElementEditorModule.extend({
	...
	fetchCache: function fetchCache(type, cacheKey, requestArgs) {
		...
		return matportalPro.ajax.addRequest('forms_panel_action_data', {
...
module.exports = matportalModules.editor.utils.Module.extend({
	elementType: null,
	...
	addEditorListener: function addEditorListener() {
		...
		if (self.onElementChange) {
			...
			matportal.channels.editor.on(eventName, function (controlView, elementView) {
	...
	addControlSpinner: function addControlSpinner(name) {
		...
		$el.find('.matportal-control-title').after('<span class="matportal-control-spinner"><i class="eicon-spinner eicon-animation-spin"></i>&nbsp;</span>');
	},
	...
	removeControlSpinner: function removeControlSpinner(name) {
		...
		$controlEl.find('.matportal-control-spinner').remove();
	},

	addSectionListener: function addSectionListener(section, callback) {
		var self = this;

		matportal.channels.editor.on('section:activated', function (sectionName, editor) {
...
var matportalPro = Marionette.Application.extend({
	config: {},
	...
	ajax: {
		...
		send: function send() {
			return matportalCommon.ajax.send.apply(matportalCommon.ajax, this.prepareArgs(arguments));
		},

		addRequest: function addRequest() {
			return matportalCommon.ajax.addRequest.apply(matportalCommon.ajax, this.prepareArgs(arguments));
		}
	},

	translate: function translate(stringKey, templateArgs) {
		return matportalCommon.translate(stringKey, null, templateArgs, this.config.i18n);
	},

	onStart: function onStart() {
		var _this = this;

		this.config = matportalProEditorConfig;

		this.initModules();

		jQuery(window).on('matportal:init', function () {
			return _this.onmatportalInit();
		}).on('matportal/connect/success/editor-pro-activate', this.onActivateSuccess);
	},

	onmatportalInit: function onmatportalInit() {
		var _this2 = this;

		matportal.on('preview:loaded', function () {
			return _this2.onmatportalPreviewLoaded();
		});
	...
	onmatportalPreviewLoaded: function onmatportalPreviewLoaded() {
		matportal.$preview[0].contentWindow.matportalPro = this;
	},
	...
	onActivateSuccess: function onActivateSuccess() {
		// Hide notice.
		matportal.noticeBar.onCloseClick();

		// Mark site connect for insert templates connect screen.
		matportal.config.library_connect.is_connected = true;

		// Mark pro is active - for `this.libraryRemoveGetProButtons`.
		matportalPro.config.isActive = true;

		matportal.notifications.showToast({
			message: matportal.translate('connected_successfully')
		});
	}
});

window.matportalPro = new matportalPro();
...
var _class = function (_matportalModules$edi) {
	_inherits(_class, _matportalModules$edi);
	...
	_createClass(_class, [{
		...
		value: function addCustomCss(css, view) {
			...
			var selector = '.matportal-element.matportal-element-' + model.get('id');

			if ('document' === model.get('elType')) {
				selector = matportal.config.settings.page.cssWrapperSelector;
			}
		...
		key: 'onmatportalInit',
		value: function onmatportalInit() {
			matportal.hooks.addFilter('editor/style/styleText', this.addCustomCss);

			matportal.on('navigator:init', this.onNavigatorInit.bind(this));
		}
	}, {
		key: 'onNavigatorInit',
		value: function onNavigatorInit() {
			matportal.navigator.indicators.customCSS = {
				icon: 'code-bold',
				settingKeys: ['custom_css'],
				title: matportalPro.translate('custom_css'),
	...
	return _class;
}(matportalModules.editor.utils.Module);
...
var _class = function (_matportalModules$edi) {
	...
	_createClass(_class, [{
		...
		key: 'onNavigatorInit',
		value: function onNavigatorInit() {
			matportal.navigator.indicators.motionFX = {
				icon: 'flash',
				title: matportalPro.translate('motion_effects'),
...
var Module = function (_matportalModules$edi) {
	_inherits(Module, _matportalModules$edi);
	...
	_createClass(Module, [{
		...
		value: function addPublishTabs() {
			var config = matportal.config.document.displaySettings,
			...
			jQuery.each(this.displaySettingsTypes, function (type, data) {
				...
				data.model = new matportalModules.editor.elements.models.BaseSettings(config[type].settings, { controls: config[type].controls });
				...
				component.addTab(type, {
					...
					viewOptions: {
						name: type,
						id: 'matportal-popup-' + type + '__controls',
						model: data.model,
						controls: data.model.controls
					},
					name: type,
					title: matportalPro.translate(type),
					description: matportalPro.translate('popup_publish_screen_' + type + '_description'),
					image: matportalPro.config.urls.modules + ('popup/assets/images/' + type + '-tab.svg')
				});
			});
		}
	}, {
		...
		value: function addPanelFooterSubmenuItems() {
			...
			jQuery.each(this.displaySettingsTypes, function (type, data) {
				matportal.getPanelView().footer.currentView.addSubMenuItem('saver-options', {
		...
		value: function addLibraryTab() {
			...
			library.addTab('templates/popups', {
				title: matportalPro.translate('popups'),
				filter: {
					source: 'remote',
					type: 'popup'
				}
		...
		value: function initIntroduction() {
			...
			this.getIntroduction = function () {
				if (!introduction) {
					introduction = new matportalModules.editor.utils.Introduction({
						introductionKey: 'popupSettings',
						dialogOptions: {
							id: 'matportal-popup-settings-introduction',
							headerMessage: '<i class="eicon-info"></i>' + matportalPro.translate('popup_settings_introduction_title'),
							message: matportalPro.translate('popup_settings_introduction_message'),
							closeButton: true,
		...
		value: function onmatportalInit() {
			...
			if ('popup' !== matportal.config.document.type) {
				return;
			}

			matportal.on('panel:init', function () {
				return _this2.onmatportalPanelInit();
			});

			matportal.saver.on('save', function () {
		...
		key: 'onmatportalPanelInit',
		value: function onmatportalPanelInit() {
			this.addPublishTabs();

			this.addPanelFooterSubmenuItems();

			if (!matportal.config.user.introduction.popupSettings) {
				this.initIntroduction();
			}
		}
	}, {
		key: 'onmatportalPreviewLoaded',
		value: function onmatportalPreviewLoaded() {
			if ('popup' !== matportal.config.document.type) {
				return;
			}
			...
			if (!matportal.config.user.introduction.popupSettings) {
				matportal.getPanelView().getCurrentPageView().on('destroy', this.onPageSettingsDestroy.bind(this));
			}
		}
	}, {
		...
		value: function onPageSettingsDestroy() {
			...
			introduction.show(matportal.getPanelView().footer.currentView.ui.settings[0]);
		...
		value: function onEditorSave() {
			...
			matportalPro.ajax.addRequest('popup_save_display_settings', {
				data: {
					settings: settings
				}
			});
...
var _class = function (_matportalModules$edi) {
	...
	_createClass(_class, [{
		...
		value: function className() {
			return _get(_class.prototype.__proto__ || Object.getPrototypeOf(_class.prototype), 'className', this).call(this) + ' matportal-popup__display-settings';
		}
	}, {
		key: 'toggleGroup',
		value: function toggleGroup(groupName, $groupElement) {
			$groupElement.toggleClass('matportal-active', !!this.model.get(groupName));
		}
	}, {
		...
		value: function onRender() {
			...
			this.children.each(function (child) {
				...
				$groupWrapper = jQuery('<div>', {
					id: 'matportal-popup__' + name + '-controls-group--' + groupName,
					class: 'matportal-popup__display-settings_controls_group'
				});

				var $imageWrapper = jQuery('<div>', { class: 'matportal-popup__display-settings_controls_group__icon' }),
				    $image = jQuery('<img>', { src: matportalPro.config.urls.modules + ('popup/assets/images/' + name + '/' + groupName + '.svg') });
	...
	return _class;
}(matportalModules.editor.views.ControlsStack);
...
module.exports = matportalModules.editor.utils.Module.extend({
	onmatportalPreviewLoaded: function onmatportalPreviewLoaded() {
		matportal.addControlView('Query', __webpack_require__(17));
	}
});
...
module.exports = matportal.modules.controls.Select2.extend({
	...
	getSelect2Placeholder: function getSelect2Placeholder() {
		return {
			id: '',
			text: matportalPro.translate('all')
		};
	},
	...
	getQueryData: function getQueryData() {
		...
		var autocomplete = matportalCommon.helpers.cloneObject(this.model.get('autocomplete'));
	...
	getSelect2DefaultOptions: function getSelect2DefaultOptions() {
		...
		return jQuery.extend(matportal.modules.controls.Select2.prototype.getSelect2DefaultOptions.apply(this, arguments), {
			ajax: {
				transport: function transport(params, success, failure) {
					...
					return matportalPro.ajax.addRequest(action, {
						data: data,
						success: success,
						error: failure
					});
	...
	getValueTitles: function getValueTitles() {
		...
		matportalCommon.ajax.loadObjects({
			action: action,
			ids: ids,
			data: data,
	...
	addControlSpinner: function addControlSpinner() {
		...
		this.$el.find('.matportal-control-title').after('<span class="matportal-control-spinner">&nbsp;<i class="eicon-spinner eicon-animation-spin"></i>&nbsp;</span>');
	},

	onReady: function onReady() {
		// Safari takes it's time to get the original select width
		setTimeout(matportal.modules.controls.Select2.prototype.onReady.bind(this));
...
module.exports = matportalModules.editor.utils.Module.extend({
	onmatportalInit: function onmatportalInit() {
		...
		matportal.addControlView('Fields_map', __webpack_require__(33));
	}
});
...
module.exports = function () {
	...
	var updateReplyToOptions = function updateReplyToOptions() {
		...
		emailFields = _.map(emailModels, function (model) {
			return {
				...
				label: matportalPro.translate('x_field', [model.get('field_label')])
			};
		});
	...
	var init = function init() {
		matportal.hooks.addAction('panel/open_editor/widget/form', onPanelShow);
	};
...
module.exports = matportalModules.editor.utils.Module.extend({
	enqueueRecaptchaJs: function enqueueRecaptchaJs(url, type) {
		if (!matportalFrontend.elements.$body.find('[src="' + url + '"]').length) {
			matportalFrontend.elements.$body.append('<scr' + 'ipt src="' + url + '" id="recaptcha-' + type + '"</scri' + 'pt>');
		}
	},
	renderField: function renderField(inputField, item) {
		inputField += '<div class="matportal-field ' + item.field_type + ' ">';
	...
	getDataSettings: function getDataSettings(item) {
		var config = matportalPro.config.forms[item.field_type],
		    srcURL = 'https://www.google.com/recaptcha/api.js?render=explicit';

		if (!config.enabled) {
			return '<div class="matportal-alert matportal-alert-info">' + config.setup_message + '</div>';
		}
		...
		return '<div class="matportal-g-recaptcha' + _.escape(item.css_classes) + '" ' + recaptchaData + '></div>';
	},
	...
	onInit: function onInit() {
		matportal.hooks.addFilter('matportal_pro/forms/content_template/item', this.filterItem);
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/recaptcha', this.renderField, 10, 2);
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/recaptcha_v3', this.renderField, 10, 2);
	}
});
...
module.exports = ElementEditorModule.extend({
	...
	updateShortcode: function updateShortcode(view) {
		...
		view.$el.find('.matportal-form-field-shortcode').focus(function () {
			this.select();
		}).val(template);
	},
	onSectionActive: function onSectionActive() {
...
module.exports = BaseIntegrationModule.extend({
	...
	updateFieldsMapping: function updateFieldsMapping() {
		...
		var remoteFields = [{
			remote_label: matportal.translate('Email'),
			remote_type: 'email',
			remote_id: 'email',
			remote_required: true
		}, {
			remote_label: matportal.translate('Name'),
			remote_type: 'text',
			remote_id: 'name',
			remote_required: false
		}, {
			remote_label: matportal.translate('Last Name'),
			remote_type: 'text',
			remote_id: 'last_name',
			remote_required: false
		}, {
			remote_label: matportal.translate('Company'),
			remote_type: 'text',
			remote_id: 'company',
			remote_required: false
		}, {
			remote_label: matportal.translate('Phone'),
			remote_type: 'text',
			remote_id: 'phone',
			remote_required: false
		}, {
			remote_label: matportal.translate('Country'),
			remote_type: 'text',
			remote_id: 'country',
			remote_required: false
		}, {
			remote_label: matportal.translate('State'),
			remote_type: 'text',
			remote_id: 'state',
			remote_required: false
		}, {
			remote_label: matportal.translate('City'),
			remote_type: 'text',
			remote_id: 'city',
			remote_required: false
		}, {
			remote_label: matportal.translate('Zip'),
			remote_type: 'text',
			remote_id: 'zip',
			remote_required: false
		}];
...
module.exports = BaseIntegrationModule.extend({
	...
	updateFieldsMapping: function updateFieldsMapping() {
		...
		var remoteFields = [{
			...
			remote_label: matportal.translate('First Name'),
			remote_type: 'text',
			...
			remote_label: matportal.translate('Organization name'),
			remote_type: 'text',
			remote_id: 'orgname',
			remote_required: false
		}];
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	renderField: function renderField(inputField, item, i, settings) {
		...
		if ('yes' === item.use_native_time) {
			itemClasses += ' matportal-use-native';
		}

		return '<input size="1" type="time"' + placeholder + ' class="matportal-field-textual matportal-time-field matportal-field matportal-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' >';
	},

	onInit: function onInit() {
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/time', this.renderField, 10, 4);
	}
});
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	renderField: function renderField(inputField, item, i, settings) {
		...
		return '<input size="1"' + min + max + placeholder + ' pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" type="date" class="matportal-field-textual matportal-date-field matportal-field matportal-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' >';
	},

	onInit: function onInit() {
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/date', this.renderField, 10, 4);
	}
});
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	renderField: function renderField(inputField, item, i, settings) {
		...
		return '<div class="matportal-field-subgroup">' + '<span class="matportal-field-option">' + '<input size="1" type="checkbox"' + checked + ' class="matportal-acceptance-field matportal-field matportal-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' > ' + label + '</span></div>';
	},

	onInit: function onInit() {
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/acceptance', this.renderField, 10, 4);
	}
});
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	renderField: function renderField(inputField, item, i, settings) {
		...
		return '<input size="1"  type="file" class="matportal-file-field matportal-field matportal-size-' + settings.input_size + ' ' + itemClasses + '" name="' + fieldName + '" id="form_field_' + i + '" ' + required + multiple + ' >';
	},

	onInit: function onInit() {
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/upload', this.renderField, 10, 4);
	}
});
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	renderField: function renderField(inputField, item, i, settings) {
		...
		itemClasses = 'matportal-field-textual ' + itemClasses;

		return '<input size="1" type="' + item.field_type + '" class="matportal-field-textual matportal-field matportal-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' ' + placeholder + ' pattern="[0-9()-]" >';
	},

	onInit: function onInit() {
		matportal.hooks.addFilter('matportal_pro/forms/content_template/field/tel', this.renderField, 10, 4);
	}
});
...
module.exports = matportal.modules.controls.Repeater.extend({
	...
	onRender: function onRender() {
		matportal.modules.controls.Base.prototype.onRender.apply(this, arguments);
		...
		self.children.each(function (view) {
			...
			view.$el.find('.matportal-repeater-row-tools').hide();
			view.$el.find('.matportal-repeater-row-controls').removeClass('matportal-repeater-row-controls').find('.matportal-control').css({
				paddingBottom: 0
			});
		});

		self.$el.find('.matportal-button-wrapper').remove();
...
module.exports = function () {
	...
	self.onTemplateIdChange = function () {
		var templateID = self.templateIdView.elementSettingsModel.get('template_id'),
		    $editButton = self.templateIdView.$el.find('.matportal-edit-template');
		...
		var editUrl = matportalConfig.home_url + '?p=' + templateID + '&matportal';
		...
		if ($editButton.length) {
			...
			$editButton = jQuery('<a />', {
				target: '_blank',
				class: 'matportal-button matportal-button-default matportal-edit-template',
				href: editUrl,
				html: '<i class="eicon-pencil" /> ' + matportalPro.config.i18n.edit_template
			});

			self.templateIdView.$el.find('.matportal-control-input-wrapper').after($editButton);
		}
	};

	self.init = function () {
		matportal.hooks.addAction('panel/open_editor/widget/template', self.onPanelShow);
	};
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	addGlobalWidget: function addGlobalWidget(id, args) {
		args = _.extend({}, args, {
			categories: [],
			icon: matportal.config.widgets[args.widgetType].icon,
	...
	createGlobalModel: function createGlobalModel(id, modelArgs) {
		var globalModel = new matportal.modules.elements.models.Element(modelArgs),
	...
	setWidgetType: function setWidgetType() {
		matportal.hooks.addFilter('element/view', function (DefaultView, model) {
		...
		matportal.hooks.addFilter('element/model', function (DefaultModel, attrs) {
			if (attrs.templateID) {
	...
	registerTemplateType: function registerTemplateType() {
		matportal.templates.registerTemplateType('widget', {
			showInLibrary: false,
			saveDialog: {
				title: matportalPro.translate('global_widget_save_title'),
				description: matportalPro.translate('global_widget_save_description')
			},
	...
	addSavedWidgetsToPanel: function addSavedWidgetsToPanel() {
		...
		_.each(matportalPro.config.widget_templates, function (templateArgs, id) {
			self.addGlobalWidget(id, templateArgs);
		});

		matportal.hooks.addFilter('panel/elements/regionViews', function (regionViews) {
	...
	addPanelPage: function addPanelPage() {
		matportal.getPanelView().addPage('globalWidget', {
	...
	saveTemplates: function saveTemplates() {
		...
		matportalCommon.ajax.addRequest('update_templates', {
			data: {
	...
	setSaveButton: function setSaveButton() {
		matportal.saver.on('before:save:publish', _.bind(this.saveTemplates, this));
		matportal.saver.on('before:save:private', _.bind(this.saveTemplates, this));
	},

	requestGlobalModelSettings: function requestGlobalModelSettings(globalModel, callback) {
		matportal.templates.requestTemplateContent('local', globalModel.get('id'), {
			success: function success(data) {
				...
				matportal.history.history.setActive(false);
				...
				matportal.history.history.setActive(true);
			}
		});
	},

	setWidgetContextMenuSaveAction: function setWidgetContextMenuSaveAction() {
		matportal.hooks.addFilter('elements/widget/contextMenuGroups', function (groups, widget) {
	...
	onmatportalFrontendInit: function onmatportalFrontendInit() {
		this.addSavedWidgetsToPanel();
	},

	onmatportalPreviewLoaded: function onmatportalPreviewLoaded(isFirst) {
		if (!isFirst) {
			return;
		}
		...
		$e.routes.register('panel/editor', 'global', function (args) {
			matportal.getPanelView().setPage('globalWidget', 'Global Editing', { editedView: args.view });
		});
	},

	onWidgetTemplateSaved: function onWidgetTemplateSaved(data) {
		matportal.history.history.startItem({
			title: matportal.config.widgets[data.widgetType].title,
			type: matportalPro.translate('linked_to_global')
		});

		var widgetModel = matportal.templates.layout.modalContent.currentView.model,
		    widgetModelIndex = widgetModel.collection.indexOf(widgetModel);

		matportal.templates.layout.hideModal();
		...
		matportal.history.history.endItem();
...
var WidgetView = matportal.modules.elements.views.Widget,
    GlobalWidgetView;
...
GlobalWidgetView = WidgetView.extend({
	...
	className: function className() {
		return WidgetView.prototype.className.apply(this, arguments) + ' matportal-global-widget matportal-global-' + this.model.get('templateID');
	},
	...
	initialize: function initialize() {
		...
		if (previewSettings) {
			...
			if (!globalSettingsLoadedStatus) {
				...
				matportalPro.modules.globalWidget.requestGlobalModelSettings(globalModel);
			}
	...
	getGlobalModel: function getGlobalModel() {
		if (!this.globalModel) {
			this.globalModel = matportalPro.modules.globalWidget.getGlobalModels(this.model.get('templateID'));
		}
	...
	unlink: function unlink() {
		...
		matportal.history.history.startItem({
			...
			type: matportalPro.translate('unlink_widget')
		});

		var newModel = new matportal.modules.elements.models.Element({
			elType: 'widget',
			widgetType: globalModel.get('widgetType'),
			id: matportal.helpers.getUniqueID(),
			settings: matportal.helpers.cloneObject(globalModel.get('settings').attributes),
			defaultEditSettings: matportal.helpers.cloneObject(globalModel.get('editSettings').attributes)
		});
...
module.exports = matportal.modules.elements.models.Element.extend({
	initialize: function initialize() {
		this.set({ widgetType: 'global' }, { silent: true });

		matportal.modules.elements.models.Element.prototype.initialize.apply(this, arguments);

		matportalFrontend.config.elements.data[this.cid].on('change', this.onSettingsChange.bind(this));
	},
	...
	initSettings: function initSettings() {
		...
		matportalFrontend.config.elements.data[this.cid] = settingsModel;

		matportalFrontend.config.elements.editSettings[this.cid] = globalModel.get('editSettings');
	},
	...
	getGlobalModel: function getGlobalModel() {
		...
		return matportalPro.modules.globalWidget.getGlobalModels(templateID);
	},
	...
	getTitle: function getTitle() {
		...
		var global = matportalPro.translate('global');
...
module.exports = matportal.modules.layouts.panel.pages.elements.views.Elements.extend({
	id: 'matportal-global-templates',
...
var GlobalWidgetsView = matportal.modules.layouts.panel.pages.elements.views.Global;

module.exports = GlobalWidgetsView.extend({
	template: '#tmpl-matportal-panel-global-widget-no-templates',

	id: 'matportal-panel-global-widget-no-templates',

	className: 'matportal-nerd-box matportal-panel-nerd-box'
});
...
module.exports = Marionette.ItemView.extend({
	id: 'matportal-panel-global-widget',

	template: '#tmpl-matportal-panel-global-widget',

	ui: {
		editButton: '#matportal-global-widget-locked-edit .matportal-button',
		unlinkButton: '#matportal-global-widget-locked-unlink .matportal-button',
		loading: '#matportal-global-widget-loading'
	},
	...
	buildUnlinkDialog: function buildUnlinkDialog() {
		...
		return matportalCommon.dialogsManager.createWidget('confirm', {
			id: 'matportal-global-widget-unlink-dialog',
			headerMessage: matportalPro.translate('unlink_widget'),
			message: matportalPro.translate('dialog_confirm_unlink'),
			position: {
				my: 'center center',
				at: 'center center'
			},
			strings: {
				confirm: matportalPro.translate('unlink'),
				cancel: matportalPro.translate('cancel')
			},
	...
	onEditButtonClick: function onEditButtonClick() {
		...
		self.ui.loading.removeClass('matportal-hidden');

		matportalPro.modules.globalWidget.requestGlobalModelSettings(editedModel, function () {
			self.ui.loading.addClass('matportal-hidden');
...
module.exports = matportalModules.editor.utils.Module.extend({
	onmatportalInit: function onmatportalInit() {
		matportal.channels.editor.on('section:activated', this.onSectionActivated);
	},
	...
	onSectionActivated: function onSectionActivated(sectionName, editor) {
		...
		editedElement.$el.toggleClass('matportal-flip-box--flipped', isSideBSection);

		var $backLayer = editedElement.$el.find('.matportal-flip-box__back');
...
module.exports = matportalModules.editor.utils.Module.extend({
	config: matportalPro.config.shareButtonsNetworks,
	...
	getNetworkClass: function getNetworkClass(networkName) {
		...
		if (matportal.config.icons_update_needed) {
...
module.exports = matportalModules.Module.extend({
	...
	getCustomFont: function getCustomFont(fontType, font) {
		matportalPro.ajax.addRequest('assets_manager_panel_action_data', {
			...
			success: function success(data) {
				if (data.font_face) {
					matportal.$previewContents.find('style:last').after('<style type="text/css">' + data.font_face + '</style>');
				}

				if (data.font_url) {
					matportal.$previewContents.find('link:last').after('<link href="' + data.font_url + '" rel="stylesheet" type="text/css">');
				}
			}
		});
	...
	onInit: function onInit() {
		matportal.channels.editor.on('font:insertion', this.onFontChange.bind(this));
	}
});
...
module.exports = function () {
	...
	self.init = function () {
		matportal.hooks.addAction('panel/open_editor/widget/post-comments', self.onPanelShow);
	};
...
module.exports = matportalModules.editor.utils.Module.extend({
	...
	onmatportalInit: function onmatportalInit() {
		if (!matportalPro.config.theme_builder) {
			return;
		}

		matportal.channels.editor.on('page_settings:preview_settings:activated', this.onSectionPreviewSettingsActive);

		matportal.addControlView('Conditions_repeater', __webpack_require__(54));

		matportal.hooks.addFilter('panel/footer/behaviors', this.addFooterBehavior);

		matportal.saver.on('save', this.onEditorSave);
	...
	saveAndReload: function saveAndReload() {
		matportal.saver.saveAutoSave({
			onSuccess: function onSuccess() {
				matportal.dynamicTags.cleanCache();
				matportal.reloadPreview();
			}
		});
	},
		...
		if (!_.isUndefined(model.changed.page_template)) {
			matportal.saver.saveAutoSave({
				onSuccess: function onSuccess() {
					...
					matportal.once('preview:loaded', function () {
						$e.route('panel/page-settings/settings');
					});
				}
			});
	...
	updatePreviewIdOptions: function updatePreviewIdOptions(render) {
		var previewType = matportal.settings.page.model.get('preview_type');
		...
		var currentView = matportal.getPanelView().getCurrentPageView(),
	...
	onmatportalPreviewLoaded: function onmatportalPreviewLoaded() {
		...
		matportal.getPanelView().on('set:page:page_settings', this.updatePreviewIdOptions);

		matportal.settings.page.model.on('change', this.onPageSettingsChange.bind(this));

		matportal.channels.editor.on('matportalThemeBuilder:ApplyPreview', this.onApplyPreview.bind(this));

		// Scroll to Editor. Timeout according to preview resize css animation duration.
		setTimeout(function () {
			matportal.$previewContents.find('html, body').animate({
				scrollTop: matportal.$previewContents.find('#matportal').offset().top - matportal.$preview[0].contentWindow.innerHeight / 2
			});
		}, 500);
	},

	setConditionsModel: function setConditionsModel() {
		var themeBuilderModuleConfig = matportalPro.config.theme_builder,
		    settings = themeBuilderModuleConfig.settings;

		this.conditionsModel = new matportalModules.editor.elements.models.BaseSettings(settings, {
	...
	onEditorSave: function onEditorSave() {
		...
		matportalPro.ajax.addRequest('theme_builder_save_conditions', {
			data: this.conditionsModel.toJSON({ remove: ['default'] }),
			success: function success() {
				matportalPro.config.theme_builder.settings.conditions = _this.conditionsModel.get('conditions');
			}
		});
	}
});
...
var _class = function (_matportalModules$com) {
	_inherits(_class, _matportalModules$com);
	...
	_createClass(_class, [{
		...
		value: function defaultTabs() {
			return {
				conditions: {
					title: matportalPro.translate('conditions'),
					View: _view2.default,
					viewOptions: {
					...
					name: 'conditions',
					description: matportalPro.translate('conditions_publish_screen_description'),
					image: matportalPro.config.urls.modules + 'theme-builder/assets/images/conditions-tab.svg'
				}
			};
		}
	}, {
		...
		value: function getTabsWrapperSelector() {
			return '#matportal-publish__tabs';
		}
	}, {
		key: 'renderTab',
		value: function renderTab(tab) {
			...
			this.layout.modal.getElements('publish').toggleClass('matportal-button-success', isLastTab);
		}
	}, {
		key: 'activateTab',
	...
	return _class;
}(matportalModules.common.ComponentModal);
...
var _class = function (_Marionette$LayoutVie) {
	...
	_createClass(_class, [{
		key: 'id',
		value: function id() {
			return 'matportal-publish';
		}
	}, {
		key: 'getTemplate',
		value: function getTemplate() {
			return Marionette.TemplateCache.get('#tmpl-matportal-component-publish');
		}
	}, {
		key: 'regions',
		value: function regions() {
			return {
				screen: '#matportal-publish__screen'
			};
		}
	}, {
		key: 'templateHelpers',
...
var _class = function (_matportalModules$com) {
	...
	_createClass(_class, [{
		...
		value: function getLogoOptions() {
			return {
				title: matportalPro.translate('publish_settings')
			};
		}
	}, {
		key: 'initModal',
		value: function initModal() {
			...
			this.modal.addButton({
				name: 'publish',
				text: matportalPro.translate('save_and_close'),
				callback: function callback() {
			...
			this.modal.addButton({
				name: 'next',
				text: matportalPro.translate('next'),
				callback: function callback() {
			...
			this.modal.getElements('next').addClass('matportal-button-success').add($publishButton).addClass('matportal-button').removeClass('dialog-button');
		}
	}]);

	return _class;
}(matportalModules.common.views.modal.Layout);
...
module.exports = inlineControlsStack.extend({
	id: 'matportal-theme-builder-conditions-view',

	template: '#tmpl-matportal-theme-builder-conditions-view',

	childViewContainer: '#matportal-theme-builder-conditions-controls',
...
module.exports = matportalModules.editor.views.ControlsStack.extend({
	activeTab: 'content',
...
module.exports = matportal.modules.controls.Repeater.extend({
	...
	initialize: function initialize() {
		matportal.modules.controls.Repeater.prototype.initialize.apply(this, arguments);

		this.config = matportalPro.config.theme_builder;
	...
	checkConflicts: function checkConflicts(model) {
		var modelId = model.get('_id'),
		    rowId = 'matportal-condition-id-' + modelId,
		    errorMessageId = 'matportal-conditions-conflict-message-' + modelId,
		    $error = jQuery('#' + errorMessageId);

		// On render - the row isn't exist, so don't cache it.
		jQuery('#' + rowId).removeClass('matportal-error');

		$error.remove();

		matportalPro.ajax.addRequest('theme_builder_conditions_check_conflicts', {
			unique_id: rowId,
			data: {
			...
			success: function success(data) {
				if (!_.isEmpty(data)) {
					jQuery('#' + rowId).addClass('matportal-error').after('<div id="' + errorMessageId + '" class="matportal-conditions-conflict-message">' + data + '</div>');
				}
			}
		});
	},
	...
	onRender: function onRender() {
		this.ui.btnAddRow.text(matportalPro.translate('add_condition'));
...
module.exports = matportal.modules.controls.RepeaterRow.extend({

	template: '#tmpl-matportal-theme-builder-conditions-repeater-row',

	childViewContainer: '.matportal-theme-builder-conditions-repeater-row-controls',

	id: function id() {
		return 'matportal-condition-id-' + this.model.get('_id');
	},
	...
	initialize: function initialize() {
		matportal.modules.controls.RepeaterRow.prototype.initialize.apply(this, arguments);
	...
	setTypeAttribute: function setTypeAttribute() {
		...
		typeView.$el.attr('data-matportal-condition-type', typeView.getControlValue());
	},
	...
	onModelChange: function onModelChange() {
		matportal.modules.controls.RepeaterRow.prototype.onModelChange.apply(this, arguments);
...
var SaverBehavior = matportal.modules.components.saver.behaviors.FooterSaver;

module.exports = SaverBehavior.extend({
	ui: function ui() {
		var ui = SaverBehavior.prototype.ui.apply(this, arguments);

		ui.menuConditions = '#matportal-panel-footer-sub-menu-item-conditions';
		ui.buttonPreviewSettings = '#matportal-panel-footer-theme-builder-button-preview-settings';
		ui.buttonOpenPreview = '#matportal-panel-footer-theme-builder-button-open-preview';
	...
	initialize: function initialize() {
		...
		matportal.settings.page.model.on('change', this.onChangeLocation.bind(this));
	},

	toggleMenuConditions: function toggleMenuConditions() {
		this.ui.menuConditions.toggle(!!matportalPro.config.theme_builder.settings.location);
	},
	...
	onRender: function onRender() {
		...
		this.ui.menuConditions = this.view.addSubMenuItem('saver-options', {
			...
			name: 'conditions',
			icon: 'eicon-flow',
			title: matportalPro.translate('display_conditions'),
			callback: function callback() {
		...
		this.ui.buttonPreview.tipsy('disable').html(jQuery('#tmpl-matportal-theme-builder-button-preview').html()).addClass('matportal-panel-footer-theme-builder-buttons-wrapper matportal-toggle-state');
	},

	onChangeLocation: function onChangeLocation(settings) {
		if (!_.isUndefined(settings.changed.location)) {
			matportalPro.config.theme_builder.settings.location = settings.changed.location;
			this.toggleMenuConditions();
		}
	},

	onClickButtonPublish: function onClickButtonPublish() {
		var hasConditions = matportalPro.config.theme_builder.settings.conditions.length,
		    hasLocation = matportalPro.config.theme_builder.settings.location,
		    isDraft = 'draft' === matportal.settings.page.model.get('post_status');
	...
	onClickButtonPreviewSettings: function onClickButtonPreviewSettings() {
		var panel = matportal.getPanelView();
		$e.route('panel/page-settings/settings');
