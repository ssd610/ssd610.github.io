...
jQuery(function () {
	matportalProAdmin.assetsManager.fontManager.init();
	matportalProAdmin.roleManager.advancedRoleManager.init();
});
...
module.exports = function () {
	...
	self.init = function () {
		jQuery(document).on('change', '.matportal-widget-template-select', function () {
			var $this = jQuery(this),
			    templateID = $this.val(),
			    $editButton = $this.parents('p').find('.matportal-edit-template'),
			    type = $this.find('[value="' + templateID + '"]').data('type');

			if ('page' !== type) {
				// 'widget' is editable only from matportal page
				$editButton.hide();

				return;
			}

			var editUrl = matportalProConfig.i18n.home_url + '?p=' + templateID + '&matportal';
...
module.exports = function (key, fieldID) {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#matportal_pro_' + key + '_button'),
			$apiKeyField: jQuery('#matportal_pro_' + key),
			$apiUrlField: jQuery('#matportal_pro_' + fieldID)
		};
	};
...
module.exports = function () {
	...
	self.selectors = {
		editPageClass: 'post-type-matportal_font',
		title: '#title',
		...
		customFontsMetaBox: '#matportal-font-custommetabox',
		closeHandle: 'button.handlediv',
		toolbar: '.matportal-field-toolbar',
		inlinePreview: '.inline-preview',
		fileUrlInput: '.matportal-field-file input[type="text"]'
	};
	...
	self.updateRowLabel = function (event, $table) {
		...
		return self.renderTemplate(self.fontLabelTemplate, {
			...
			preview: '<span style="' + previewStyle.styleAttribute + '">matportal is making the web beautiful</span>',
			toolbar: toolbarHtml
		});
	};
...
module.exports = {
	...
	selectors: {
		uploadBtnClass: 'matportal-upload-btn',
		clearBtnClass: 'matportal-upload-clear-btn',
		uploadBtn: '.matportal-upload-btn',
		clearBtn: '.matportal-upload-clear-btn',
		inputURLField: '.matportal-field-file input[type="text"]'
	},
	...
	setUploadParams: function setUploadParams(ext, name) {
		...
		uploader.param('uploadTypeCaller', 'matportal-admin-font-upload');
...
module.exports = {
	selectors: {
		...
		repeaterField: '.matportal-field-repeater'
	},
...
module.exports = function () {
	...
	self.fetchFonts = function () {
		...
		jQuery.post(ajaxurl, {
			action: 'matportal_pro_admin_fetch_fonts',
			kit_id: kitID,
			_nonce: self.cache.$button.data('nonce')
		}).done(function (data) {
			if (data.success) {
...
var CustomIcons = function (_matportalModules$Vie) {
	_inherits(CustomIcons, _matportalModules$Vie);
	...
	_createClass(CustomIcons, [{
		...
		value: function getDefaultSettings() {
			return {
				...
				classes: {
					editPageClass: 'post-type-matportal_icons',
					editPhp: 'edit-php',
					hasIcons: 'matportal--has-icons'
				},
				selectors: {
					editPageClass: 'post-type-matportal_icons',
					title: '#title',
					metaboxContainer: '#matportal-custom-icons-metabox',
					metabox: '.matportal-custom-icons-metabox',
					closeHandle: 'button.handlediv',
					iconsTemplate: '#matportal-icons-template',
					dataInput: '#matportal_custom_icon_set_config',
					dropzone: '.zip_upload',
				...
				templates: {
					...
					header: jQuery('#matportal-custom-icons-template-header').html(),
					footer: jQuery('#matportal-custom-icons-template-footer').html(),
					duplicatePrefix: jQuery('#matportal-custom-icons-template-duplicate-prefix').html()
				}
			};
		}
	}, {
		...
		value: function bindEvents() {
			...
			matportalCommon.elements.$document.on('click', '.remove', triggerDelete);
		...
		value: function prepareIconName(icon) {
			...
			return matportalCommon.helpers.upperCaseWords(iconName);
		}
	}, {
		key: 'getCreatedOn',
		...
		value: function enqueueCSS(url) {
			if (!matportalCommon.elements.$document.find('link[href="' + url + '"]').length) {
				matportalCommon.elements.$document.find('link:last').after('<link href="' + url + '" rel="stylesheet" type="text/css">');
			}
		}
	}, {
		key: 'setData',
		...
		value: function renderIconList(config) {
			...
			return config.icons.map(function (icon) {
				...
				return matportalCommon.compileTemplate(iconTemplate, data);
			}).join('\n');
		}
	}, {
		key: 'renderIcons',
		value: function renderIcons(config) {
			...
			$metabox.html('');
			$metaboxContainer.prepend(matportalCommon.compileTemplate(header, config));
			$metabox.append('<ul>' + this.renderIconList(config) + '</ul>');
			$metaboxContainer.append(matportalCommon.compileTemplate(footer, this.getCreatedOn()));
		}
	}, {
		key: 'onTitleInput',
		...
		value: function showAlertDialog(id, message) {
			...
			return matportalCommon.dialogsManager.createWidget('alert', alertData).show();
		}
	}, {
		key: 'onSuccess',
		...
		value: function onInit() {
			...
			var $body = matportalCommon.elements.$body,
...
var DropZoneField = function (_matportalModules$Vie) {
	_inherits(DropZoneField, _matportalModules$Vie);
	...
	_createClass(DropZoneField, [{
		key: 'getDefaultSettings',
		value: function getDefaultSettings() {
			var baseSelector = '.matportal-dropzone-field';
			return {
				droppedFiles: false,
				selectors: {
					...
					browseButton: baseSelector + ' .matportal--dropzone--upload__browse',
					postId: '#post_ID'
				},
				classes: {
					drag: 'is-dragover',
		...
		value: function handleUpload() {
			...
			matportalCommon.ajax.send('ajax', {
				data: ajaxData,
				cache: false,
		...
		value: function onInit() {
			...
			matportalCommon.elements.$document.trigger('onDropzoneLoaded', [this]);
		}
	}]);
...
var _class = function (_matportalModules$Vie) {
	...
	_createClass(_class, [{
		...
		value: function getDefaultSettings() {
			return {
				selectors: {
					button: '#matportal_pro_fa_pro_validate_button',
					kitIdField: '#matportal_font_awesome_pro_kit_id'
				}
			};
		}
	}, {
...
module.exports = function () {
	...
	self.cacheElements = function () {
		this.cache = {
			$checkBox: jQuery('input[name="matportal_exclude_user_roles[]"]'),
			$advanced: jQuery('#matportal_advanced_role_manager')
		};
	};
	self.bindEvents = function () {
	...
	self.init = function () {
		if (!jQuery('body').hasClass('matportal_page_matportal-role-manager')) {
			return;
		}
		this.cacheElements();
		this.bindEvents();
...
module.exports = function () {
	var selectors = {
		templateTypeInput: '#matportal-new-template__form__template-type',
		locationWrapper: '#matportal-new-template__form__location__wrapper',
		postTypeWrapper: '#matportal-new-template__form__post-type__wrapper'
	};
	...
	var setElements = function setElements() {
		jQuery.each(selectors, function (key, selector) {
			key = '$' + key;
			elements[key] = matportalNewTemplate.layout.getModal().getElements('content').find(selector);
		});
	};
	...
	this.init = function () {
		if (!window.matportalNewTemplate) {
			return;
		}

		// Make sure the modal has already been initialized
		matportalNewTemplate.layout.getModal();
