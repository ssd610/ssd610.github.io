...
/* global jQuery, matportalGutenbergSettings */
(function ($) {
  'use strict';

  var matportalGutenbergApp = {
    cacheElements: function cacheElements() {
      this.ismatportalMode = matportalGutenbergSettings.ismatportalMode;
      this.cache = {};
      this.cache.$gutenberg = $('#editor');
      this.cache.$switchMode = $($('#matportal-gutenberg-button-switch-mode').html());
      this.cache.$gutenberg.find('.edit-post-header-toolbar').append(this.cache.$switchMode);
      this.cache.$switchModeButton = this.cache.$switchMode.find('#matportal-switch-mode-button');
      this.toggleStatus();
      this.buildPanel();
      var self = this;
    ...
    buildPanel: function buildPanel() {
      ...
      if (!$('#matportal-editor').length) {
        self.cache.$editorPanel = $($('#matportal-gutenberg-panel').html()); // TODO: `editor-block-list__layout` class for WP < 5.3 support.
        ...
        self.cache.$editorPanelButton = self.cache.$editorPanel.find('#matportal-go-to-edit-page-link');
        ...
        if (self.ismatportalMode) {
          matportalCommon.dialogsManager.createWidget('confirm', {
            ...
            onConfirm: function onConfirm() {
              ...
              wpEditor.editPost({
                gutenberg_matportal_mode: false
              });
              wpEditor.savePost();
              self.ismatportalMode = !self.ismatportalMode;
              self.toggleStatus();
            }
          }).show();
        } else {
    ...
    redirectWhenSave: function redirectWhenSave() {
      ...
      setTimeout(function () {
        if (wp.data.select('core/editor').isSavingPost()) {
          ...
          location.href = matportalGutenbergSettings.editLink;
        }
      }, 300);
    },
    animateLoader: function animateLoader() {
      this.cache.$editorPanelButton.addClass('matportal-animate');
    },
    toggleStatus: function toggleStatus() {
      jQuery('body').toggleClass('matportal-editor-active', this.ismatportalMode).toggleClass('matportal-editor-inactive', !this.ismatportalMode);
    },
    init: function init() {
      var self = this;
      setTimeout(function () {
