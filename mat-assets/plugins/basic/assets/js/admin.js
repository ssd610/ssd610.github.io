...
(function ($) {
  var matportalAdmin = matportalModules.ViewModule.extend({
    maintenanceMode: null,
    config: matportalAdminConfig,
    getDefaultElements: function getDefaultElements() {
      var elements = {
        $switchMode: $('#matportal-switch-mode'),
        $goToEditLink: $('#matportal-go-to-edit-page-link'),
        $switchModeInput: $('#matportal-switch-mode-input'),
        $switchModeButton: $('#matportal-switch-mode-button'),
        $matportalLoader: $('.matportal-loader'),
        $builderEditor: $('#matportal-editor'),
        $importButton: $('#matportal-import-template-trigger'),
        $importArea: $('#matportal-import-template-area'),
        $settingsForm: $('#matportal-settings-form'),
        $settingsTabsWrapper: $('#matportal-settings-tabs-wrapper')
      };
      elements.$settingsFormPages = elements.$settingsForm.find('.matportal-settings-form-page');
      elements.$activeSettingsPage = elements.$settingsFormPages.filter('.matportal-active');
    ...
    toggleStatus: function toggleStatus() {
      var ismatportalMode = this.ismatportalMode();
      matportalCommon.elements.$body.toggleClass('matportal-editor-active', ismatportalMode).toggleClass('matportal-editor-inactive', !ismatportalMode);
    },
    bindEvents: function bindEvents() {
      ...
      self.elements.$switchModeButton.on('click', function (event) {
        ...
        if (self.ismatportalMode()) {
          ...
          if (!$wpTitle.val()) {
            $wpTitle.val('matportal #' + $('#post_ID').val());
          }
          ...
          $(document).on('heartbeat-tick.autosave', function () {
            matportalCommon.elements.$window.off('beforeunload.edit-post');
      ...
      $('div.notice.matportal-message-dismissed').on('click', 'button.notice-dismiss, .matportal-button-notice-dismiss', function (event) {
        event.preventDefault();
        $.post(ajaxurl, {
          action: 'matportal_set_admin_notice_viewed',
          notice_id: $(this).closest('.matportal-message-dismissed').data('notice_id')
        });
        var $wrapperElm = $(this).closest('.matportal-message-dismissed');
      ...
      $('#matportal-clear-cache-button').on('click', function (event) {
        event.preventDefault();
        var $thisButton = $(this);
        $thisButton.removeClass('success').addClass('loading');
        $.post(ajaxurl, {
          action: 'matportal_clear_cache',
          _nonce: $thisButton.data('nonce')
        }).done(function () {
          $thisButton.removeClass('loading').addClass('success');
        });
      });
      $('#matportal-library-sync-button').on('click', function (event) {
        event.preventDefault();
        var $thisButton = $(this);
        $thisButton.removeClass('success').addClass('loading');
        $.post(ajaxurl, {
          action: 'matportal_reset_library',
          _nonce: $thisButton.data('nonce')
        }).done(function () {
          $thisButton.removeClass('loading').addClass('success');
        });
      });
      $('#matportal-replace-url-button').on('click', function (event) {
        event.preventDefault();
        var $this = $(this),
            $tr = $this.parents('tr'),
        ...
        $.post(ajaxurl, {
          ...
          matportalCommon.dialogsManager.createWidget('alert', {
            message: response.data
          }).show();
        });
      });
      $('#matportal_upgrade_fa_button').on('click', function (event) {
        event.preventDefault();
        ...
        matportalCommon.dialogsManager.createWidget('confirm', {
          ...
          onConfirm: function onConfirm() {
            ...
            $.post(ajaxurl, $updateButton.data()).done(function (response) {
              ...
              $('#matportal_upgrade_fa_button').parent().append(response.data.message);
      ...
      $('select.matportal-rollback-select').on('change', function () {
        var $this = $(this),
            $rollbackButton = $this.next('.matportal-rollback-button'),
      ...
      $('.matportal-rollback-button').on('click', function (event) {
        event.preventDefault();
        var $this = $(this);
        matportalCommon.dialogsManager.createWidget('confirm', {
      ...
      $('.matportal_css_print_method select').on('change', function () {
        var $descriptions = $('.matportal-css-print-method-description');
        $descriptions.hide();
    ...
    initTemplatesImport: function initTemplatesImport() {
      if (!matportalCommon.elements.$body.hasClass('post-type-matportal_library')) {
        return;
      }
      ...
      $importButton.on('click', function () {
        $('#matportal-import-template-area').toggle();
      });
    },
    initMaintenanceMode: function initMaintenanceMode() {
      var MaintenanceMode = __webpack_require__(603);

      this.maintenanceMode = new MaintenanceMode();
    },
    ismatportalMode: function ismatportalMode() {
      return !!this.elements.$switchModeInput.val();
    },
    animateLoader: function animateLoader() {
      this.elements.$goToEditLink.addClass('matportal-animate');
    },
    ...
    goToSettingsTab: function goToSettingsTab(tabName) {
      ...
      this.elements.$activeSettingsPage.removeClass('matportal-active');
      this.elements.$activeSettingsTab.removeClass('nav-tab-active');
      var $activeTab = this.elements.$settingsTabs.filter('#matportal-settings-' + tabName);
      $activePage.addClass('matportal-active');
    ...
    roleManager: {
      selectors: {
        body: 'matportal-role-manager',
        row: '.matportal-role-row',
        label: '.matportal-role-label',
        excludedIndicator: '.matportal-role-excluded-indicator',
        excludedField: 'input[name="matportal_exclude_user_roles[]"]',
        controlsContainer: '.matportal-role-controls',
        toggleHandle: '.matportal-role-toggle',
  ...
  $(function () {
    window.matportalAdmin = new matportalAdmin();
    matportalCommon.elements.$window.trigger('matportal/admin/init');
  });
})(jQuery);
...
module.exports = matportalModules.ViewModule.extend({
  getDefaultSettings: function getDefaultSettings() {
    return {
      selectors: {
        modeSelect: '.matportal_maintenance_mode_mode select',
        maintenanceModeTable: '#tab-maintenance_mode table',
        maintenanceModeDescriptions: '.matportal-maintenance-mode-description',
        excludeModeSelect: '.matportal_maintenance_mode_exclude_mode select',
        excludeRolesArea: '.matportal_maintenance_mode_exclude_roles',
        templateSelect: '.matportal_maintenance_mode_template_id select',
        editTemplateButton: '.matportal-edit-template',
        maintenanceModeError: '.matportal-maintenance-mode-error'
      },
      classes: {
        isEnabled: 'matportal-maintenance-mode-is-enabled'
      }
    };
  },
  getDefaultElements: function getDefaultElements() {
  ...
  handleTemplateSelectChange: function handleTemplateSelectChange() {
    ...
    var editUrl = matportalAdmin.config.home_url + '?p=' + templateID + '&matportal';
  ...
  onInit: function onInit() {
    matportalModules.ViewModule.prototype.onInit.apply(this, arguments);
    matportalCommon.elements.$window.on('matportal/admin/init', this.onAdminInit);
  }
});
