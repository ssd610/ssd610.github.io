...
InnerTabsBehavior = Marionette.Behavior.extend({
  ...
  onChildviewControlTabClicked: function onChildviewControlTabClicked(childView) {
    ...
    matportal.getPanelView().updateScrollbar();
  }
});
...
var EditorModule = matportalModules.Module.extend({
  onInit: function onInit() {
    var _this = this;

    var $window = jQuery(window);
    $window.on('matportal:init-components', this.onmatportalInitComponents.bind(this));
    $window.on('matportal:loaded', function () {
      _this.onmatportalLoaded();

      matportal.on('document:loaded', _this.onDocumentLoaded.bind(_this));
    });
    $window.on('matportal:init', this.onmatportalReady);
  },
  // TODO: Delete as soon as possible.
  getEditorControlView: function getEditorControlView(name) {
    var editor = matportal.getPanelView().getCurrentPageView();
  ...
  onmatportalReady: function onmatportalReady() {
    this.onmatportalInit();
    matportal.on('frontend:init', this.onmatportalFrontendInit.bind(this)).on('preview:loaded', this.onmatportalPreviewLoaded.bind(this));
  }
});

EditorModule.prototype.onmatportalLoaded = function () {};

EditorModule.prototype.onmatportalInit = function () {};

EditorModule.prototype.onmatportalPreviewLoaded = function () {};

EditorModule.prototype.onDocumentLoaded = function () {};

EditorModule.prototype.onmatportalFrontendInit = function () {};

EditorModule.prototype.onmatportalInitComponents = function () {};
...
var _default = /*#__PURE__*/function (_matportalModules$Mod) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initDialog() {
      ...
      this.getDialog = function () {
        if (!dialog) {
          ...
          dialog = matportalCommon.dialogsManager.createWidget(settings.dialogType, settings.dialogOptions);
    ...
    value: function setViewed() {
      ...
      matportalCommon.ajax.addRequest('introduction_viewed', {
        data: {
...
ControlsStack = Marionette.CompositeView.extend({
  classes: {
    popover: 'matportal-controls-popover'
  },
  activeTab: null,
  activeSection: null,
  className: function className() {
    return 'matportal-controls-stack';
  },
  templateHelpers: function templateHelpers() {
    return {
      elementData: matportal.getElementData(this.model)
    };
  },
  ...
  ui: function ui() {
    return {
      tabs: '.matportal-panel-navigation-tab',
      reloadButton: '.matportal-update-preview-button'
    };
  },
  ...
  initialize: function initialize() {
    this.initCollection();
    this.listenTo(matportal.channels.deviceMode, 'change', this.onDeviceModeChange);
  },
  initCollection: function initCollection() {
    this.collection = new Backbone.Collection(_.values(matportal.mergeControlsSettings(this.getOption('controls'))));
  },
  ...
  getChildView: function getChildView(item) {
    ...
    return matportal.getControlView(controlType);
  },
  ...
  getNamespaceArray: function getNamespaceArray() {
    return [matportal.getPanelView().getCurrentPageName()];
  },
  openActiveSection: function openActiveSection() {
    ...
    if (activeSectionView[0]) {
      activeSectionView[0].$el.addClass('matportal-open');
      var eventNamespace = this.getNamespaceArray();
      eventNamespace.push(activeSection, 'activated');
      matportal.channels.editor.trigger(eventNamespace.join(':'), this);
    }
  },
  onRenderCollection: function onRenderCollection() {
  ...
  onDeviceModeChange: function onDeviceModeChange() {
    if ('desktop' === matportal.channels.deviceMode.request('currentMode')) {
      this.$el.removeClass('matportal-responsive-switchers-open');
    }
  },
  onChildviewControlSectionClicked: function onChildviewControlSectionClicked(childView) {
    var isSectionOpen = childView.$el.hasClass('matportal-open');
  ...
  onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
    if ('desktop' === device) {
      this.$el.toggleClass('matportal-responsive-switchers-open');
    }
  }
});
...
BaseSettingsModel = Backbone.Model.extend({
  ...
  initialize: function initialize(data, options) {
    ...
    self.controls = matportal.mergeControlsSettings(options.controls);
    self.validators = {};
    ...
    _.each(self.controls, function (control) {
      ...
      if ('object' === (0, _typeof2.default)(control.default)) {
        defaults[controlName] = matportalCommon.helpers.cloneObject(control.default);
      } else {
      ...
      if (undefined !== attrs[controlName] && isMultipleControl && !_.isObject(attrs[controlName]) && !hasDynamicSettings) {
        matportal.debug.addCustomError(new TypeError('An invalid argument supplied as multiple control value'), 'InvalidElementData', 'Element `' + (self.get('widgetType') || self.get('elType')) + '` got <' + attrs[controlName] + '> as `' + controlName + '` value. Expected array or object.');
        ...
        if (!(attrs[field.name] instanceof Backbone.Collection)) {
          attrs[field.name] = new Backbone.Collection(attrs[field.name], {
            model: function model(attributes, options) {
              ...
              if (!attributes._id) {
                attributes._id = matportal.helpers.getUniqueID();
              }
  ...
  getStyleControls: function getStyleControls(controls, attributes) {
    var self = this;
    controls = matportalCommon.helpers.cloneObject(self.getActiveControls(controls, attributes));
    var styleControls = [];
    jQuery.each(controls, function () {
      var control = this,
          controlDefaultSettings = matportal.config.controls[control.type];
  ...
  getActiveControls: function getActiveControls(controls, attributes) {
    ...
    jQuery.each(controls, function (controlKey, control) {
      if (matportal.helpers.isActiveControl(control, attributes)) {
  ...
  clone: function clone() {
    return new BaseSettingsModel(matportalCommon.helpers.cloneObject(this.attributes), matportalCommon.helpers.cloneObject(this.options));
  },
  ...
  parseDynamicSettings: function parseDynamicSettings(settings, options, controls) {
    var self = this;
    settings = matportalCommon.helpers.cloneObject(settings || self.attributes);
    options = options || {};
    controls = controls || this.controls;
    jQuery.each(controls, function () {
      var control = this,
      ...
      if (undefined === dynamicSettings) {
        dynamicSettings = matportal.config.controls[control.type].dynamic;
      }
      ...
      try {
        dynamicValue = matportal.dynamicTags.parseTagsText(valueToParse, dynamicSettings, matportal.dynamicTags.getTagDataContent);
      } catch (error) {
        if (matportal.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR !== error.message) {
          throw error;
        }
        ...
        matportal.dynamicTags.refreshCacheFromServer(function () {
  ...
  toJSON: function toJSON(options) {
    ...
    return matportalCommon.helpers.cloneObject(data);
  }
});
...
var Container = /*#__PURE__*/function (_ArgsObject) {
  ...
  function Container(args) {
    ...
    if (!_this.document) {
      _this.document = matportal.documents.getCurrent();
    }
  ...
  (0, _createClass2.default)(Container, [{
    ...
    value: function isEditable() {
      return 'edit' === matportal.channels.dataEditMode.request('activeMode') && 'open' === this.document.editor.status;
    }
  }, {
    key: "isDesignable",
    value: function isDesignable() {
      return matportal.userCan('design') && this.isEditable();
    }
  }]);
  return Container;
}(_argsObject.default);
